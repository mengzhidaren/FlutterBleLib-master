...
class InsertValueDialogState extends State<InsertValueDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle contextTextStyle = Theme.of(context).textTheme.body1;
  ...
  _dataAsBytes() {
    if (_currentDataType == DataType.STRING) {
      return Utf8Encoder().convert(_controller.text);
//      return UTF8.encode(_controller.text);
    }
    return new List<int>()
      ..add(int.parse(_controller.text, radix: _calculateRadix(),
          onError: (error) {
  ...
  _radioGroupCollection() => DataType.values
      .map(
        (dataType) => new RadioListTile<DataType>(
