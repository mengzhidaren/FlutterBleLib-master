...
public class BleHelper {
    ...
    void destroyClient() {
        // Subscriptions
        if (adapterStateChangesSubscription != null && !adapterStateChangesSubscription.isDisposed()) {
    ...
    private Disposable monitorAdapterStateChanges(Context context) {
        return new RxBleAdapterStateObservable(context)
                .map(new Function<RxBleAdapterStateObservable.BleAdapterState, BleData.BluetoothStateMessage>() {
                    ...
                    @Override
                    public void accept(BleData.BluetoothStateMessage state) throws Exception {
                ...
                public Observable<RxBleConnection> apply(final RxBleConnection rxBleConnection) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        return rxBleConnection
                                ...
                                .flatMapObservable(new Function<Integer, ObservableSource<RxBleConnection>>() {
                                    @Override
                                    public ObservableSource<RxBleConnection> apply(Integer integer) {
        ...
        final Disposable subscription = connect
                .subscribe(new Consumer<RxBleConnection>() {
                    ...
                    public void accept(RxBleConnection rxBleConnection) throws Exception {
                        Device deviceWrapper = new Device(device, rxBleConnection);
    ...
    void characteristicsForDevice(final String deviceId,
        ...
        final Device device = getDeviceOrReject(deviceId, errorAction);
        if (device == null) {
            errorAction.onError(new ServiceNotFoundException("Cannot find service : device == null"));
            return;
        }
